...
def get_default_config():
    ...
    cfg.data.workers = 10  # number of data loading workers
    ...
    cfg.loss.focal = CN()
    cfg.loss.focal.gamma = 2.0  # parameter `gamma`
    cfg.loss.focal.weight_all_f = 1.  # weight to balance focal loss w.r.t. all features
    cfg.loss.focal.weight_f = 1.  # weight to balance focal loss
    cfg.loss.focal.weight_x = 0.  # weight to balance cross entropy loss
    cfg.loss.arc = CN()
    cfg.loss.arc.s = 30.
    cfg.loss.arc.m = 0.1
    ...
    cfg.loss.supcon = CN()
    cfg.loss.supcon.weight_s = 1.
    cfg.loss.supcon.temperature = 0.07
    cfg.loss.supcon.base_temperature = 0.07
    ...
    cfg.test.visrankactiv = False  # visualize ranked results and its activations maps(only available when cfg.test.evaluate=True)
    ...
    cfg.test.maskthr = 0.7  # threshold for masking activations
...
def imagedata_kwargs(cfg):
    return {
        ...
        'self_supervised': 'supcon' in cfg.loss.name
    }
...
def engine_run_kwargs(cfg):
    return {
        ...
        'vispartmap': cfg.test.vispartmap,
