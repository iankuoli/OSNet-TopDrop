...
class ImageArcFocalDropBatchDropBotFeaturesEngine(Engine):
    ...
    def __init__(self, datamanager, model, optimizer, metric_loss,
                 ...
                 weight_all_f=1, weight_f=0, weight_x=1,
        ...
        super(ImageArcFocalDropBatchDropBotFeaturesEngine, self).__init__(datamanager, model, optimizer, scheduler, use_gpu)
    ...
    def forward_backward(self, data, epoch=0):
        ...
        loss_all_f = self._compute_loss(self.criterion_all_f, embed, pids)
        # Global loss
        embed_f = self.arc_f(features, pids)
        loss_f = self._compute_loss(self.criterion_f, embed_f, pids)
        loss_x = self._compute_loss(self.criterion_x, outputs, pids)
        # DropBot loss
        embed_db_f = self.arc_db_f(db_features, pids)
        loss_db_f = self._compute_loss(self.criterion_db_f, embed_db_f, pids)
        loss_db_x = self._compute_loss(self.criterion_db_x, db_prelogits, pids)
        # Batch DropBot loss
        embed_b_db_f = self.arc_b_db_f(b_db_features, pids)
        loss_b_db_f = self._compute_loss(self.criterion_b_db_f, embed_b_db_f, pids)
        loss_b_db_x = self._compute_loss(self.criterion_b_db_x, b_db_prelogits, pids)

        loss = self.weight_all_f * loss_all_f + \
