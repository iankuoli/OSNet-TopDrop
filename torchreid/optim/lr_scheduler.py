...
AVAI_SCH = ['single_step', 'multi_step', 'cosine', 'warmup_db', 'warmup_sb', 'warmup_db_osnet']


def warmup_db_osnet(ep, stepsize):
    ep += 1
    if ep < stepsize[0]:
        lr = 5e-4*(ep//5+1)
    elif ep < stepsize[1]:
...
def build_lr_scheduler(optimizer, lr_scheduler='single_step', stepsize=1, gamma=0.1, max_epoch=1):
    ...
    if lr_scheduler == 'single_step':
    ...
    elif lr_scheduler == 'warmup_db_osnet':
        if not isinstance(stepsize, list):
            raise TypeError(
        ...
        warmup_lambda = lambda epoch: warmup_db_osnet(epoch, stepsize)
